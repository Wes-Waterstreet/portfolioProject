...
I will be using the java map to represent my component because it uses the same pair entries. The representation will have a pair with a string and integer entry instead of K and V types. It will be interpreted as a map with the first entry of the pair the media and the second the rating for the media.
